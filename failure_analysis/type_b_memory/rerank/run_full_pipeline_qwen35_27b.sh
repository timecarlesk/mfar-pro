...
$PY failure_analysis/type_b_memory/rerank/train_memory/run_stage1_stage2.py \
  --splits train --model qwen3.5:27b --endpoints $EP

$PY failure_analysis/type_b_memory/rerank/train_memory/extract_rerouted.py \
  --splits train --detect_model qwen3.5:27b
...
$PY failure_analysis/type_b_memory/rerank/detect_route/run_stage1_stage2.py \
  --splits val test --model qwen3.5:27b --endpoints $EP --memory_version memory_v1

# ═══════════════════════════════════════════════════════════
# Step 4: Rerank v1 on val → verify → memory v2
# ═══════════════════════════════════════════════════════════
$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py score \
  --splits val --model qwen3.5:27b --endpoints $EP --memory_version memory_v1
$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py merge \
  --splits val --model qwen3.5:27b --alpha 0.7 --memory_version memory_v1
...
$PY failure_analysis/type_b_memory/rerank/scoring/verify.py update-memory \
  --splits val --model qwen3.5:27b --memory_version memory_v1

# ═══════════════════════════════════════════════════════════
# Step 5: Stage 1+2 on val/test with memory v2 + KG
# ═══════════════════════════════════════════════════════════
$PY failure_analysis/type_b_memory/rerank/detect_route/run_stage1_stage2.py \
  --splits val test --model qwen3.5:27b --endpoints $EP --memory_version memory_v2

$PY failure_analysis/type_b_memory/rerank/detect_route/run_stage1_stage2.py \
  --splits val test --model qwen3.5:27b --endpoints $EP --memory_version memory_kg

# ═══════════════════════════════════════════════════════════
# Step 6: Rerank scoring — all conditions on test
# ═══════════════════════════════════════════════════════════
$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py score \
  --splits test --model qwen3.5:27b --endpoints $EP --memory_version memory_v1
$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py merge \
  --splits val test --model qwen3.5:27b --alpha_sweep 0.3 0.5 0.7 1.0 --memory_version memory_v1

$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py score \
  --splits test --model qwen3.5:27b --endpoints $EP --memory_version memory_v2
$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py merge \
  --splits val test --model qwen3.5:27b --alpha_sweep 0.3 0.5 0.7 1.0 --memory_version memory_v2

$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py score \
  --splits test --model qwen3.5:27b --endpoints $EP --memory_version memory_kg
$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py merge \
  --splits val test --model qwen3.5:27b --alpha_sweep 0.3 0.5 0.7 1.0 --memory_version memory_kg

$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py score \
  --splits test --model qwen3.5:27b --endpoints $EP --memory_version memory_v1 --no_memory
$PY failure_analysis/type_b_memory/rerank/scoring/rerank.py merge \
  --splits val test --model qwen3.5:27b --alpha_sweep 0.3 0.5 0.7 1.0 --memory_version memory_v1 --no_memory
