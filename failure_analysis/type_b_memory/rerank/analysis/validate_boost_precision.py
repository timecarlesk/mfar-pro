...
def load_qwen3_cache(split, detect_model=None):
    search_paths = []
    ...
    for mtag in model_tags:
        search_paths.extend([
            os.path.join(CACHE_DIR, "stage12", mtag, "memory_v2", f"qwen3_cache_{split}.jsonl"),
            os.path.join(CACHE_DIR, "stage12", mtag, "memory_v1", f"qwen3_cache_{split}.jsonl"),
            os.path.join(CACHE_DIR, "stage12", mtag, "shared", f"qwen3_cache_{split}.jsonl"),
        ])
    if not model_tag or model_tag == "qwen3_8b":
        search_paths.extend([
            os.path.join(CACHE_DIR, "stage12", "memory_v2", f"qwen3_cache_{split}.jsonl"),
            ...
            os.path.join(CACHE_DIR, "qwen3_8b", f"qwen3_cache_{split}.jsonl"),
            os.path.join(CACHE_DIR, f"qwen3_cache_{split}.jsonl"),
        ])
    cache = {}
...
def main():
    ...
    for split in args.splits:
        ...
        qwen3_cache = load_qwen3_cache(split, detect_model=args.detect_model)
